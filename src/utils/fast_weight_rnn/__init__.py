...
mod_causal_dot_product_cuda = load(
    name="fast_rnn_forward", sources=[filename], verbose=True)
mod_causal_dot_backward_cuda = load(
    name="fast_rnn_backward", sources=[filename], verbose=True)

causal_dot_product_cuda = mod_causal_dot_product_cuda.fast_rnn_forward
causal_dot_backward_cuda = mod_causal_dot_backward_cuda.fast_rnn_backward


class FastRNN(torch.autograd.Function):
    ...
    def forward(ctx, Z, K, V, W, beta, h0):
        ...
        ctx.save_for_backward(rnn_out, Z, K, V, beta, V_old, W, h_init)
    ...
    def backward(ctx, grad_out):
        ...
        rnn_out, Z, K, V, beta, V_old, W, h0 = ctx.saved_tensors
        ...
        FastRNN.dot_backward[Z.device.type](
            K.data,
            V.data,
            beta.data,
            V_old.data,
...
if __name__ == '__main__':
    ...
    out1 = fast_rnn(z1, k1, v1, W1, beta1, h0)
    print("done.")
    ...
    for pos in range(slen):
        ...
        rec_part = torch.bmm(W, h.unsqueeze(2)).squeeze()
        # h = torch.sigmoid(rec_part + z_2[pos])
        h = F.softmax(rec_part + z_2[pos], dim=-1)
    ...
    rand_targets = torch.empty(
        slen * bsz * n_head, dtype=torch.long, device='cuda').random_(v_dim)
    loss1 = loss_func(out1.reshape(-1, v_dim), rand_targets)
    # grad
    # loss1 = out1.sum()
    z1.retain_grad()
    k1.retain_grad()
    ...
    loss2 = loss_func(out2.reshape(-1, v_dim), rand_targets)
    # loss2 = out2.sum()
    z2.retain_grad()
    k2.retain_grad()
    v2.retain_grad()
